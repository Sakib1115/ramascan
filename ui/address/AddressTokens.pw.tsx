...
const test = base.extend({
  page: async({ page }, use) => {
    ...
    await page.route(API_URL_TOKENS + '?type=RAMA-20', (route) => route.fulfill({
      status: 200,
      body: JSON.stringify(response20),
    }));
    await page.route(API_URL_TOKENS + '?type=RAMA-721', (route) => route.fulfill({
      status: 200,
      body: JSON.stringify(response721),
    }));
    await page.route(API_URL_TOKENS + '?type=RAMA-1155', (route) => route.fulfill({
      status: 200,
...
base.describe('update balances via socket', () => {
  ...
  test('', async({ mount, page, createSocket }) => {
    ...
    await page.waitForResponse(API_URL_TOKENS + '?type=RAMA-20');
    await page.waitForResponse(API_URL_TOKENS + '?type=RAMA-721');
    await page.waitForResponse(API_URL_TOKENS + '?type=RAMA-1155');
