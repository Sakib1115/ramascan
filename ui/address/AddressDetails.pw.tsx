...
const API_URL_TOKENS_RAMA20 = buildApiUrl('address_tokens', { hash: ADDRESS_HASH }) + '?type=RAMA-20';
const API_URL_TOKENS_RAMA721 = buildApiUrl('address_tokens', { hash: ADDRESS_HASH }) + '?type=RAMA-721';
const API_URL_TOKENS_ER1155 = buildApiUrl('address_tokens', { hash: ADDRESS_HASH }) + '?type=RAMA-1155';
const hooksConfig = {
  router: {
...
test('token', async({ mount, page }) => {
  ...
  await page.route(API_URL_TOKENS_RAMA20, async(route) => route.fulfill({
    status: 200,
    body: JSON.stringify(tokensMock.erc20List),
  }), { times: 1 });
  await page.route(API_URL_TOKENS_RAMA721, async(route) => route.fulfill({
    status: 200,
